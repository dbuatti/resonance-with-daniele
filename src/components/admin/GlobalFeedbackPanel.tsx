...
import {
  ...
  Music, Zap, Sparkles, Brain, AlertTriangle, CheckCircle2,
  PieChart as PieChartIcon, BarChart3, MapPin, LineChart as LineChartIcon,
  UserPlus, EyeOff, ListMusic, Heart, BarChart, CalendarCheck,
  Calendar, MessageSquare, ExternalLink, SearchCode, Frown, MessageSquareText
} from "lucide-react";
...
interface AiInsights {
  ...
  critical_friction: string[];
  strategic_advice: string;
  repertoire_analysis: string | { specific_requests: string[]; thematic_patterns: string[]; summary: string };
}

const getEventDate = (f: FeedbackRow): string => {
  const d = Array.isArray(f.events) ? f.events[0]?.date : f.events?.date;
  return d || f.created_at;
};

const getMode = (values: Array<string | null>): string => {
  const counts: Record<string, number> = {};
  values.forEach((v) => { if (v) counts[v] = (counts[v] || 0) + 1; });
  return Object.entries(counts).sort((a, b) => b[1] - a[1])[0]?.[0] || "N/A";
};

const GlobalFeedbackPanel: React.FC = () => {
  ...
  const { data: allFeedback, isLoading: loadingFeedback } = useQuery<FeedbackRow[]>({
  ...
  const uniqueMonths = useMemo(() => {
    ...
    allFeedback.forEach((f) => {
      months.add(format(parseISO(getEventDate(f)), "yyyy-MM"));
    });
  ...
  const filteredFeedback = useMemo(() => {
    ...
    if (selectedFilter.startsWith("month:")) {
      ...
      return allFeedback.filter((f) => {
        const date = parseISO(getEventDate(f));
  ...
  const { data: savedAiSummary } = useQuery({
    queryKey: ["savedGlobalAiSummary", selectedFilter],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  const analyzeMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["savedGlobalAiSummary", selectedFilter] });
    },
  ...
  const stats = useMemo(() => {
    ...
    filteredFeedback.forEach((f) => {
      ...
      const monthKey = format(startOfMonth(parseISO(getEventDate(f))), "MMM yyyy");
    ...
    return {
      total, avgScore, marketingSources, repertoire, trendData, returningCount, newCount,
      npsChartData, sentimentChartData,
      topPrice: getMode(filteredFeedback.map((f) => f.price_point)),
      topTimeSlot: getMode(filteredFeedback.map((f) => f.time_slot_rating)),
      topPreferredTime: getMode(filteredFeedback.map((f) => f.best_times_ongoing?.[0] || null)),
      topFrequency: getMode(filteredFeedback.map((f) => f.attendance_frequency)),
      ongoingTimes,
      frequencies,
    };
  }, [filteredFeedback]);
  ...
  const selectedLabel = selectedFilter === "all"
    ? "Global Feedback"
    : `Feedback: ${format(parseISO(`${selectedFilter.split(":")[1]}-01`), "MMMM yyyy")}`;
  const selectedDescription = selectedFilter === "all"
    ? "Aggregated insights from every session."
    : "Analyze how sessions landed during this month.";
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <h2 className="text-3xl font-black font-lora tracking-tight">{selectedLabel}</h2>
          <p className="text-base text-muted-foreground font-medium">{selectedDescription}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card className="bg-primary text-primary-foreground rounded-[2rem] shadow-xl border-none p-8">
          <div className="flex justify-between items-start">
            <div className="space-y-1"><p className="text-[10px] font-black uppercase tracking-widest opacity-70">Average NPS</p><p className="text-6xl font-black tracking-tighter">{stats?.avgScore.toFixed(1) || "0.0"}</p></div>
        ...
        <Card className="rounded-[2.5rem] shadow-xl border-none p-8 bg-card md:col-span-2">
          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2 mb-4">
            <LineChartIcon className="h-3 w-3" /> NPS Trend Over Time
          </p>
        ...
        <Card className="rounded-[2.5rem] shadow-xl border-none p-8 bg-accent text-accent-foreground">
          <div className="flex justify-between items-start">
            <div className="space-y-1"><p className="text-[10px] font-black uppercase tracking-widest opacity-70">Total Responses</p><p className="text-6xl font-black tracking-tighter">{stats?.total || 0}</p></div>
      ...
      {!filteredFeedback || filteredFeedback.length === 0 ? (
        <Card className="p-24 text-center border-dashed border-4 rounded-[3rem]">
          <Heart className="h-16 w-16 text-muted-foreground mx-auto mb-4 opacity-10" />
          <p className="text-xl font-bold text-muted-foreground">No feedback found for this timeframe.</p>
        </Card>
      ) : (
        <div className="space-y-10">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <Card className="rounded-[2.5rem] shadow-xl border-none p-8 bg-card">
              <CardTitle className="text-xl font-black font-lora flex items-center gap-2 mb-6"><BarChart className="h-5 w-5 text-primary" /> Marketing Sources</CardTitle>
              <div className="space-y-4">
                {Object.entries(stats?.marketingSources || {}).sort((a, b) => (b[1] as number) - (a[1] as number)).map(([source, count]) => (
                  <div key={source} className="space-y-1">
                    <div className="flex justify-between text-xs font-bold"><span>{source}</span><span>{count}</span></div>
                    <Progress value={((count as number) / stats!.total) * 100} className="h-1.5 bg-primary/10" />
                  </div>
                ))}
              </div>
            </Card>
            <Card className="rounded-[2.5rem] shadow-xl border-none p-8 bg-card">
              ...
              <div className="space-y-4">
                {Object.entries(stats?.ongoingTimes || {}).sort((a, b) => (b[1] as number) - (a[1] as number)).slice(0, 5).map(([time, count]) => (
            ...
            <Card className="rounded-[2.5rem] shadow-xl border-none p-8 bg-card">
              <CardTitle className="text-xl font-black font-lora flex items-center gap-2 mb-6"><UserCheck className="h-5 w-5 text-primary" /> Attendance Frequency</CardTitle>
              <div className="space-y-4">
                {Object.entries(stats?.frequencies || {}).sort((a, b) => (b[1] as number) - (a[1] as number)).map(([freq, count]) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="p-6 space-y-6">
                    {filteredFeedback.map((f, i) => (
                      <div key={i} className="group relative space-y-2 border-b border-border/50 pb-6 last:border-0">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">— {f.is_anonymous ? "Anonymous Member" : (f.profiles?.first_name || "Legacy Member")}</p>
                      </div>
                    ))}
                  </div>
                </ScrollArea>
            ...
            <Card className="rounded-[2.5rem] shadow-xl border-none overflow-hidden">
              ...
              <CardContent className="p-0">
                <ScrollArea className="h-[400px]">
                  <div className="p-6 space-y-6">
                    {filteredFeedback.filter((f) => f.improvements).map((f, i) => (
                      <div key={i} className="group relative space-y-2 border-b border-border/50 pb-6 last:border-0">
                        <p className="text-sm italic font-medium leading-relaxed pr-10">"{f.improvements}"</p>
                        <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">— {f.is_anonymous ? "Anonymous Member" : (f.profiles?.first_name || "Legacy Member")}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2 bg-muted/30 p-4 rounded-2xl">
                        {(selectedResponse.interest_next_month || []).map((date: string, i: number) => (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2 bg-muted/30 p-4 rounded-2xl">
                        {(selectedResponse.best_times_ongoing || []).map((time: string, i: number) => (
...
export default GlobalFeedbackPanel;