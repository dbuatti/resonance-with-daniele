...
import { Trash2, Edit as EditIcon, Search, MessageSquare, Send, Mail, Phone, Users as UsersIcon } from "lucide-react";
...
const AdminPeopleHub: React.FC = () => {
  ...
  const { data: submissions, isLoading: loadingSubmissions } = useQuery<InterestSubmission[], Error>({
  ...
  return (
    <AdminWorkbenchShell
      title="People"
      description="Manage your singers, survey insights, and interest leads."
      badge="Community"
      actions={
        <>
          <Button
    ...
    >
      <Tabs value={activeTab} onValueChange={setTab} className="space-y-8">
        <TabsList className="bg-muted/50 p-1 rounded-2xl h-14 w-full flex overflow-x-auto no-scrollbar">
          <TabsTrigger value="members" className="rounded-xl font-bold data-[state=active]:bg-background data-[state=active]:shadow-sm"><UsersIcon className="h-4 w-4 mr-2 shrink-0" /> Members</TabsTrigger>
          <TabsTrigger value="survey" className="rounded-xl font-bold data-[state=active]:bg-background data-[state=active]:shadow-sm"><Search className="h-4 w-4 mr-2 shrink-0" /> Survey Insights</TabsTrigger>
          <TabsTrigger value="leads" className="rounded-xl font-bold data-[state=active]:bg-background data-[state=active]:shadow-sm"><Mail className="h-4 w-4 mr-2 shrink-0" /> Interest Leads</TabsTrigger>
        </TabsList>
              ...
              <CardContent className="p-0">
                ...
                {filteredProfiles.length === 0 && (
                  <div className="p-20 text-center">
                    <UsersIcon className="h-16 w-16 text-muted-foreground mx-auto mb-4 opacity-10" />
        ...
        <TabsContent value="survey" className="space-y-6">
          {loadingProfiles ? null : <SurveyMetricsCard profiles={profiles || []} />}
        </TabsContent>
                    ...
                    {!loadingSubmissions && submissions?.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={5} className="py-16 text-center">
                          <p className="text-lg font-bold text-muted-foreground font-lora">No interest submissions yet.</p>
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
