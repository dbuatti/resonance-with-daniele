-- 1. Get the Event ID for May 24 and an Admin ID
DO $$ 
DECLARE 
    may_event_id UUID;
    admin_id UUID;
BEGIN
    ...
    IF may_event_id IS NOT NULL AND admin_id IS NOT NULL THEN
        ...
        INSERT INTO public.marketing_task_status (admin_id, event_id, task_id, is_completed)
        VALUES (admin_id, may_event_id, 'update-insta-bio', false)
        ON CONFLICT DO NOTHING;
        
        -- 5. Ensure the Email Regulars task is visible in your checklist
        INSERT INTO public.marketing_task_status (admin_id, event_id, task_id, is_completed)
        VALUES (admin_id, may_event_id, 'email-regulars', false)
        ON CONFLICT DO NOTHING;
        
        RAISE NOTICE 'Successfully updated tasks and promo for event %', may_event_id;
    ELSE
